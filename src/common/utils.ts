...
export function getHoroscope(date: Date): string {
  ...
  for (const z of zodiacRanges) {
    ...
    if (startMonth < endMonth) {
      ...
      if (afterStart || beforeEnd) return z.sign; // Capricorn case
    }
  }
...
const CHINESE_ZODIAC_RANGES: ZodiacRange[] = [
...
export function getZodiac(date: Date): string {
  ...
  for (const z of CHINESE_ZODIAC_RANGES) {
    const start = new Date(z.start).getTime();
    const end = new Date(z.end).getTime();
